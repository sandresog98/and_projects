<?php
/**
 * AND PROJECTS APP - Editar Reunión
 */

if (!hasPermission('reuniones', 'editar')) {
    setFlashMessage('error', 'No tiene permisos para editar reuniones');
...
$pageTitle = 'Editar: ' . $reunion['titulo'];
$pageSubtitle = 'Modificar información de la reunión';
...
$errors = [];
// Separar ubicación y enlace si están combinados
$ubicacionActual = $reunion['ubicacion'] ?? '';
$enlaceVirtual = '';
if (strpos($ubicacionActual, ' | ') !== false) {
    list($ubicacionActual, $enlaceVirtual) = explode(' | ', $ubicacionActual, 2);
} elseif (filter_var($ubicacionActual, FILTER_VALIDATE_URL) || strpos($ubicacionActual, 'http') === 0) {
    $enlaceVirtual = $ubicacionActual;
    $ubicacionActual = '';
}
...
$formData = [
    ...
    'finalidad' => $reunion['finalidad'] ?? '',
    'insights' => $reunion['insights'] ?? ''
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            ...
            setFlashMessage('success', 'Reunión actualizada correctamente');
            header('Location: ' . uiModuleUrl('reuniones', 'ver', ['id' => $id]));
            exit;
            
        } catch (Exception $e) {
            $errors[] = 'Error al actualizar la reunión: ' . $e->getMessage();
        }
    }
}
?>
    ...
    <div class="col-lg-8">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h6 class="mb-0"><i class="bi bi-pencil me-2"></i>Editar Reunión</h6>
                <a href="<?= uiModuleUrl('reuniones', 'ver', ['id' => $id]) ?>" class="btn btn-sm btn-outline-secondary">
                ...
                <form method="POST">
                    <div class="row g-3">
                        ...
                        <div class="col-12">
                            ...
                            <input type="text" name="titulo" class="form-control" value="<?= htmlspecialchars($formData['titulo']) ?>" required>
                        </div>
                        
                        <div class="col-12">
                            <label class="form-label">Descripción</label>
                            <textarea name="descripcion" class="form-control" rows="2"><?= htmlspecialchars($formData['descripcion']) ?></textarea>
                        </div>
                        
                        <div class="col-12">
                            <label class="form-label">Finalidad / Objetivos</label>
                            <textarea name="finalidad" class="form-control" rows="2"><?= htmlspecialchars($formData['finalidad']) ?></textarea>
                        </div>
                        ...
                        <div class="col-md-8" id="ubicacionContainer">
                            ...
                            <input type="text" name="ubicacion" class="form-control" value="<?= htmlspecialchars($formData['ubicacion']) ?>">
                        </div>
                        
                        <div class="col-md-8" id="enlaceContainer" style="display: none;">
                            <label class="form-label">Enlace de la reunión virtual</label>
                            <input type="url" name="enlace_virtual" class="form-control" value="<?= htmlspecialchars($formData['enlace_virtual']) ?>">
                        </div>
                        ...
                        <!-- Insights -->
                        <div class="col-12 mt-4">
                            <h6 class="text-muted mb-3"><i class="bi bi-lightbulb me-2"></i>Insights / Notas</h6>
                        </div>
                        
                        <div class="col-12">
                            <label class="form-label">Notas de la reunión</label>
                            <textarea name="insights" class="form-control" rows="4" placeholder="Puntos importantes, decisiones, tareas..."><?= htmlspecialchars($formData['insights']) ?></textarea>
                        </div>
                        ...
                        <div class="col-12 mt-4">
                            ...
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    <?php if (hasPermission('reuniones', 'eliminar')): ?>
                                ...
                                <div class="d-flex gap-2">
                                    <a href="<?= uiModuleUrl('reuniones', 'ver', ['id' => $id]) ?>" class="btn btn-outline-secondary">Cancelar</a>
...
function confirmarEliminar() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.href = '<?= uiModuleUrl('reuniones', 'eliminar', ['id' => $id]) ?>';
        }
    });
}

// Filtrar proyectos cuando cambie la empresa (sin recargar página)
function filtrarProyectosPorEmpresa(limpiarSeleccion = true) {
    ...
    // Limpiar selección actual solo si se indica
    if (limpiarSeleccion) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Aplicar filtro inicial sin limpiar la selección actual
    filtrarProyectosPorEmpresa(false);
});
</script>
